...
### Next unit: shared HRV/RHR card consistency

Fix the shared selected-night/history reads and metric card before adding Health destinations. Overview/Sleep/Health currently read ungated HRV/RHR via readDay; Health history reads unversioned metric_series. OBMetricCard ignores readiness/reason and OBRangePill draws an unsupported fixed24px band from only a scalar baseline. Refine canonical Paper metric card3ESH and adjacent screens, then implement one shared truthful status/value presentation with no invented range.

DATA brief `night-scalar-cross-screen-next.txt` is prepared, not launched. Existing data author4c5b8d20 has the six-file scope in isolated `night-scalar-data-clone`; its accepted data files match main and can continue without resetting the clone. Reuse selected payload and pure receipt rules inreadDay; do not call two history isolates per refresh. Add exact job identity aliases and nap receipt. HRV/RHR readMetricHistory should reuse typed same-algorithm history; other metrics unchanged. Unknown/provisional/stale baseline must not produce a trusted delta. UI will need the shared status treatment because existing cards ignore readiness. No physiological computation or schema/pin change.

Discovery reports6e2589de/eb8a3d8c andfd038def are retained. After shared consistency, finish Health hub parity (Paper3FVE): respiration RSA and skin_temp_z are already fetched and dropped; skin temperature must be SD, not°C. Weight is journal_metric, notprofileBMR. Circadian has stored analytics/legacy detail. AutomaticVO2was removed, but manual-entry persistence/UI remains genuine edge work. Imported BP/core-temperature has real storage. No missing-repository-method deferrals. Full hub/body/rhythm/history/coverage and method/quality education remain open. Then B69.
