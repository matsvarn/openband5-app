...
Latest accepted commit **0b6a955** avoids unused SeriesCodec expansion in selected-cycle reads. Focused48, required1624 tests and88-item analysis passed; fresh reader review accepted. Previous accepted units: gap history **c62ee9e** (1620 required,43 inspected goldens,64 native PNGs), observation counts **4b5b92c**, measured cycle nights **c9c7a7b**, raw cycle **9f78b21**. Full evidence is in IMPLEMENTATION_VERIFICATION.md and Git. Do not rebuild them.
...
### Next unit: latest-night comparison and legacy retirement

Discovery **e68e0ada/ae43f26f** is read. Select latest actual night independently per metric, never medianSeries.latest. Named preceding21 civil-day reference excludes latest and includes day−21; it can extend before the displayed year and include valid nights from unassigned long intervals. Same-cycle-day uses earlier valid periods/source nights within the explicitly displayed year, at least3 periods. References abstain independently. Use pinned analytics mean/z; show physical-unit difference from the named mean, keep z/count/date/source in disclosure. Undefined/zero-SD z stays absent. A long/unreadable interval may withhold cycle-day comparison while retaining the real night and valid21-day reference. No phase/fertility claims.

Initial Paper comparison only: canonical4PO7/4PTT on artboards4PO4/4PTS; main4PP4/4PUL;3752x full-scroll4PZ9/4Q1K. All six inspected and exact JSX/index in task `cycle-comparison-paper/`. Page title `Vergleich` fits2x. Reuses sourced-card header/value hierarchy and window control; stacked large-text rows. Missing/error/info/date variants, typed contracts, registry and implementation are pending. No app fixture added. Draft synthetic latest56bpm/51ms; prior same-day50/52/54 and45/47/49 gives mean52/47, delta+4 and z2. Latest-period sparse arrays+2/+3 give trailing21 counts17/15, means56.176470588/49.666666667, deltas−0.176470588/+1.333333333 and z−0.099360357/+0.527832327 in a Python arithmetic probe; verify independently against pinned analytics before use.

After median+comparison migration, retire legacy `CycleTab`, analytical ui2 `_CycleHistory` and its tests/imports. Preserve OpenBand raw `_CycleHistory`. `getCycle` remains used by `rough_night.dart` to infer luteal phase in legacy SleepDetail reached from ui2 Home/Wellness/MetricDetail; remove/migrate those consumers before deleting getCycle. Do not touch crossday derivation `_lutealFlags` in a UI refactor. Legacy overlay uses120 newest rows and MAX<=algo, not the new exact-algo contract.

### Remaining supported queue

- Training: list actions/editor already accepted **ad82c97/daa5c0e**. New/saved archive aliases map to existing editor/list; save-failure Paper state still needed. Open: definition copyB65/archiveB69, template reorder/replace/superset/load-following, historyB20/B81, non-run starts reaching legacy WorkoutScreen. Discovery **c2978810/db1b579a**, alias **f56f12cd**.
...
- Release source: app0.9.31+67, schema65, algo90; protocolfe1464db98b84ac4d3ce6175d54ada11356d6c62 and analytics1fa8144a5e3b728ce91eeed6ecbc15d482933b44 match lock. Six shipping widget/watch configs0.9.31/67; three RunnerTests0.9.29/65 need final reconciliation. Required checks use Flutter3.41.6, manifest and native gallery; goldens only test/openband_goldens, never test/goldens.
