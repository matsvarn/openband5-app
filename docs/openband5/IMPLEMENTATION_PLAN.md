...
### Next unit: optional observed-gap history

Source discoverye68e0ada/4cd0098c is read. New Paper main4L7S/dark4LBD and12 missing/off/error variants are rendered and inspected, exact14 exports in task `cycle-gaps-paper/index.json`. Not yet registered or implemented. Existing sourced trend-card geometry, zero-origin bars, selected actual interval dates, no24/38 clinical guides. Header/controls/contrast/spacing inspected. Info, large variants, root/settings changes and shared chart registration still needed before UI delegation. DATA may proceed from this established contract after the preceding commit.

Use existing readCycle and lengthReviewEnabled storage. Closed consecutive contributing starts only, as-of filtered, civil-day difference. Exclude current open elapsed duration. Keep display threshold12 and existing kCycleMaxObservedGapDays60; duplicate/unreadable starts withhold, never0-day bars. Tracking disabled and optional display off have separate reasons. EstimatesEnabled/situation do not gate historical gaps. Toggle in existing settings preserves other fields; use existing commit-versus-refresh receipt/retry. No new storage or metric algorithms. Chart selection/access to long histories remains to finalize before UI brief.
