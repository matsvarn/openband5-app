...
Latest accepted commit **bd650cc** completes latest-night comparison and retires legacy cycle screens. Previous **0668e1e** adds twelve-month cycle-day medians. Prior **0b6a955** avoids unused SeriesCodec expansion in selected-cycle reads. Focused48, required1624 tests and88-item analysis passed; fresh reader review accepted. Previous accepted units: gap history **c62ee9e** (1620 required,43 inspected goldens,64 native PNGs), observation counts **4b5b92c**, measured cycle nights **c9c7a7b**, raw cycle **9f78b21**. Full evidence is in IMPLEMENTATION_VERIFICATION.md and Git. Do not rebuild them.
...
### Current unit: HRV/RHR detail parity and stored-source honesty

No implementation assigned yet. Lead owns Paper. Read-only source audits **e7f571fa/6eadeacc** and **b5877b58** are complete. Production info is a no-op, hardcoded baseline explanation is unsupported, history ignores version/source stamps and async selected-day generations. Lead verified scalar HRV can be selected by the producer from session/robust/whole-night estimates; never silently substitute cycle session HRV. Preserve the delivered overnight HRV flow. Decisions in task `hrv-detail-design-decisions.md`.

Paper3I5L had an empty36px status-bar placeholder; it is removed. Header uses canonical64px/44pt controls, redundant subtitle removed. Canonical hero3I95 no longer has a fabricated range pill. Canonical chart3I9I has a zero-origin axis, scalar baseline line and the actual15fixture nights, with no invented27/30. Propagated into3I5L as4TN2/4TND; new dark4TO5 uses proper dark card/canvas/icon tokens. Spacing/type/contrast/alignment/fit inspected. Exact PNG/JSX snapshots and dark descendant map are in `hrv-detail-paper/`; `refined-light`/`refined-dark` are latest, `top-fixed` predates full refinement. Necessary state variants, typed-contract decisions and delegation are next; this is Paper-only, not screen acceptance.
