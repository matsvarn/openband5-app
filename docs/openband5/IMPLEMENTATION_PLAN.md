...
Latest accepted commit **0668e1e** adds twelve-month cycle-day medians. Prior **0b6a955** avoids unused SeriesCodec expansion in selected-cycle reads. Focused48, required1624 tests and88-item analysis passed; fresh reader review accepted. Previous accepted units: gap history **c62ee9e** (1620 required,43 inspected goldens,64 native PNGs), observation counts **4b5b92c**, measured cycle nights **c9c7a7b**, raw cycle **9f78b21**. Full evidence is in IMPLEMENTATION_VERIFICATION.md and Git. Do not rebuild them.
...
- Next B65 discovery **c2978810/8b432a14** is accepted for scope. Existing library/detail → copy icon → seeded existing definition editor → original library, copied id unselected. Reuse create-only UUID/replay handling and refresh/hidden-created notice; distinct typed detail result must not select source. Copy only known typed metadata, retain `copiedFrom: source.id`, never history/sets/planned loads or arbitrary retained JSON. Preset load/repetition basis stays unset. No draft table or second picker. Lead preview in `exercise-copy-paper/` has fourteen inspected PNG/JSX refs: canonical4SUC/4SUQ on4SUB/4SUP; source4SV0/4SW6; incomplete preset draft4SXC/4SZF. Proposed name suffix ` · Kopie` matches template duplication and remains editable. Disabled action matches existing40% background alpha with full-opacity label; Paper color-mix was unsupported and replaced with exact token-derived RGBA. Added custom-copy4T6C/4T8F, save-error4TAI/4TCM and375px2x4TEQ/4TGT. Large-text fixture uses source name Curl, hence Curl · Kopie; other fields remain the complete dumbbell definition. No copy implementation assigned before current comparison commit; return-library4TIW/4TK6 also inspected, both copies unselected and source unchanged. Existing-detail header propagation still needed before copy acceptance.
