...
Temperature critical follow-up94e09a05: do NOT infer quantity from metric_series_version.source. That unversioned stamp is skipped for partial/emptyseries and can describe another payload even at the same algo. Add nullable source to day_result, written atomically on every putDayResult; old nulls remain unknown, no invented backfill. Schema65→66 required for this future unit. Old unambiguous importedwhoop_export payload can identify Celsius. Celsius card/history must remain usable; do not hide it behind an SD-only card. No implementation yet. Respiration fullsuite1971/analysis103/manifest127/703 pass; native still running.

Next prerequisite assigned: day-result-source worker6f6a25fd/65f3e30a (GrokPi600s), isolated day-result-source-clone, soleowner lib/data/db.dart + test/openband_day_result_source_test.dart. Outcome is exact-row nullable source provenance, additive schema66 with no backfill; no UI/physiology changes. This source-storage foundation can proceed independently of respiration nativeacceptance. Lead owns review/integration/checks/commit.
