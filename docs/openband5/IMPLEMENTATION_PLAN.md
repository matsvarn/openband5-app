...
- **Journal persistence repair ready for commit:** `journal-patch` Grok Cursor85505d55 implemented five paths; fresh reviewer f09c2f19/92cbaf5e accepted source. Single-field transaction preserves sibling answers and all existing dose times; SQLite3.18-compatible UPDATE-then-INSERT. Added new-key INSERT and independent INSERT/UPDATE-abort tests. Main240 OpenBand+affected tests, analyzer/manifest passed; source diff inspected. No UI or schema change. Full dated editor and other legacy writers remain queued. Next integrate notifications.
