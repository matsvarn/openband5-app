...
Next action after the unit commit: send `/private/tmp/openband-alpin-20260919/fable-rethink-brief.txt` using read-only `fable-consult` in task `/private/tmp/openband-scope-rethink-20260921`. Ask for the smallest coherent product, what to finish/simplify/postpone, and a proportional verification/Paper workflow. Discuss its recommendation with Mats before any implementation resumes.
