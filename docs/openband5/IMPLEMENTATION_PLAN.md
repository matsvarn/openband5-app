...
Supporting source discovery e68e0ada/bb624e37 completed and read. Exact-algo date-bounded SQL already exists in readCycleMeasurements; recentDayResults120 uses max<=90 and must not migrate. All start periods are available, but selected read truncates to120civil nights. Cross-cycle range must be separate. Source window and latest-in-selected-versus-global remain lead decisions; no implementation or Paper yet.

Discovery **e68e0ada / eeee6340** and earlier343bee3a/ff87cc30 reports remain in task. Optional observed-gap history uses existing opt-in `lengthReviewEnabled`, at least12 gaps, any gap>60 withholds; no unsourced24/38 clinical guides. Cross-cycle medians/comparisons need a cross-cycle query reusing accepted night parsing. Source window choice and Paper contract are still open; selected-cycle120-night cap alone is not that query.

Source follow-up e68e0ada/27b34baa is complete/read. Current readCycleMeasurements decodes full payloads and expands series on UI isolate, although parseCycleNightSource only uses imported, sleep_source, sleep.window and two clinical envelopes. db/health_export documents ~88KB/day and ~298MB RSS for400 bundles. Caffeine already uses json_valid/json_extract. Prefer a shared compact exact-version SQL projection preserving JSON types and correction receipts when widening the reader; avoid full-bundle batch reads. Multi-path json_extract returns a JSON array preserving booleans/envelope shapes (verify with SQLite before adopting). Do not substitute scalar rhr/rmssd columns. Product history window is still undecided; a count of periods alone can span years after missed starts.
