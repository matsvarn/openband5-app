...
## Cycle-day medians · 21 September 2026

`Zyklus → Zyklustage` groups stored RHR and session HRV by recorded cycle day within an explicit twelve-calendar-month window. The fixed end anchor supports whole-window paging and a date picker without leap-year drift. Each median needs two distinct recorded periods; a chart needs three qualifying days. Missing slots remain gaps, and each metric retains its own selected day, contributor count, actual night bounds, algorithm and source envelope. Partial coverage is labelled. No-start, disabled, unreadable, long-interval, insufficient-history and read failures have separate states.

The repository reads exact algorithm90 results and correction receipts in one SQLite snapshot, decoding raw payloads in32-row isolate batches. The shared reader preserves the existing120-night measurement path. The transaction spans the isolate batches and serializes other database work during this read. Pinned analytics supplies median and the disclosed robust spread estimate; undefined or zero spread stays absent. Stored quality scores are not uncertainty intervals. No schema, algorithm, dependency or version change.

Removing or restoring a start and refreshing its derived view remain separate outcomes. Failed restoration retains the undo identity; committed writes are not replayed on refresh retry. Pending receipts survive return to the root. Date, navigation and retry controls are guarded while restoring. Invalid year-boundary windows are refused. Fresh reviewers and lead review closed source-window grouping, gap provenance, busy-state and receipt findings.

- Flutter3.41.6 required OpenBand suite: **1702 passed**; required analysis **91 items, no issues**. Manifest **126 blocks / 597 screens**. Logs and results in `build/cycle-medians-acceptance/`. No production changes after these checks.
- DATA22aa37e7/320edec6: **22 contract tests** under America/Los_Angeles. Repository754495a9/d0068d26: **23 store tests plus101 existing selected/raw tests**. UIc18dd914/d1fdc029: **127 affected widget tests**,12-item analysis clean. Fresh reviewers f4c51855/37a3db0b,7db41698/3db12cb1 and32e77379/caeee52a accepted corrected source. Lead inspected actual source/diffs and underlying logs.
- All **37 changed/new golden PNGs** inspected, including final source disclosure and2x captures. Goldens remain only in `test/openband_goldens/`. Existing keyboard capture retained. Integrated hashes in `/private/tmp/openband-alpin-20260919/cycle-medians-integrated-hashes.json`.
- `python3 tool/ui_review.py capture --flow cycle-medians` and dedicated `--small`: **46 checkpoints each**. Pro `build/ui-review/cycle-medians-pro-20260921` PASS133.99s at393×852; mini `cycle-medians-mini-20260921` PASS139.75s at375×812. **All92 PNGs**, run.json, frames.json and HTML indexes inspected. Includes actual root/back, changed-date confirmation with root date retained, earlier year, chart gap selection, missing/partial/error/retry, disabled/settings, unreadable/history, committed remove/restore refresh failures, restore retry, light/dark and2x top/bottom with complete scrollable disclosure and pinned close.
- Paper Phase3 main4MX3/4N1K, canonical window4MWU/4N4Q and median4N0F/4N4Z. All changed renders inspected. `cycle-medians-paper/full-index.json` holds66 exact PNG/JSX exports:38 real variants,4 canonical artboards and24 adjacent-screen updates. Shared sourced charts use typed samples without fabricated dates; large-text captions stack beneath value/unit. Existing root routes and measured charts use the same blocks.

Synthetic fixture and simulator proof only. Latest-night comparisons and legacy analytical destinations remain open. No personal data, physical-device, Bluetooth or physiological validation. Nothing pushed.


## Cycle latest-night comparison · 21 September 2026

`Zyklus → Vergleich` shows the latest stored RHR and session HRV independently inside a selected twelve-month window. Each physical-unit difference names its reference: at least two valid nights in the preceding21 civil days, or at least three earlier recorded cycles on the same cycle day. References abstain independently. Actual dates, counts, algorithm, night bounds and uncertainty stay available through the shared info/source sheets. No phase or significance claim. Missing, unassigned, long, unreadable, unavailable, partial, disabled and read-error states remain distinct.

The repository reads exact-algorithm results and corrections in one database snapshot, including only the required preceding-window padding. Unused padding cannot become latest or inflate displayed coverage. Pinned analytics owns mean/z; undefined z stays absent. Root selection survives comparison paging and calendar changes. Equal estimate bounds display one date. The migrated legacy CycleTab and its analytical history/tests/export were removed; raw OpenBand entry history and separate getCycle/rough-night consumers remain.

- Required Flutter3.41.6 OpenBand suite: **1778 passed**. Three subsequent focused coverage cases passed; comparison/shared-info suite **35 passed**. Required analysis **95 items, no issues**. Logs in `build/cycle-comparison-acceptance/` and Mats task `comparison-final-*`.
- DATA43 focused tests and93 existing tests passed. Fresh DATA reviewer9c164361/c8056eee and UI reviewer65c21037/88c550b9 plus finalbe70ebe7 were reviewed by the lead against actual source/diffs/logs. Corrected source grouping, missing-vs-unavailable state, unused-padding counts, navigation/capture locators and valid test fixtures before acceptance.
- Legacy affected tests28 passed/88 intentional absent-golden skips. The pre-existing gallery inventory failure was corrected with two documented route/Alpin-block exclusions; all10 tokens tests now pass. Existing first-sync coverage and registry were verified; the scanner and gallery were not weakened.
- All **36 changed/new golden PNGs** inspected. Small320 has settled top/bottom views, and2x cards/sheets keep usable scrolling and fixed close. Goldens only in `test/openband_goldens/`.
- Dedicated Pro `build/ui-review/cycle-comparison-pro-20260921-v2`: **54 checkpoints, PASS98.09s**,393×852. Dedicated mini `build/ui-review/cycle-comparison-mini-20260921`: **54 checkpoints, PASS154.95s**,375×812. All **108 native PNGs**, run.json, frames.json and HTML indexes inspected. Actual root/back, independent metric dates, calendar cancel/confirm, retained root date, earlier window, disabled/settings/return, missing/partial/error/retry, source actions, light/dark and2x top/bottom covered. First Pro attempt failed a broad date test locator and is not acceptance evidence.
...
## HRV and resting-pulse detail · 21 September 2026

All existing HRV/RHR detail callers now open the Alpin night-scalar family. It shows the actual stored selected-night value and7/30/90 civil-day history at that row's algorithm version. Missing days stay gaps. Corrupt JSON retains its independently stored version anchor. Pending, failed, unproven and outdated correction receipts withhold the current value; the last stored value, source, window, computation and previous baseline remain behind Info. Older and partial results are labelled. Only a complete current result with an explicitly trusted stored baseline gets a delta. Seven-night bars are14px wide;30/90 retain their denser widths. No inferred range, fabricated samples or physiological algorithm change.

Source reads use one SQLite snapshot and bounded32-row off-isolate payload projections. Sleep receipt identity/revision, published algorithm/computation time, explicit correction source and second-precision night bounds must agree. Nap jobs gate the same values. Imported rows without a job remain readable; an import cannot prove a correction receipt. Refresh, calculation and source-identity changes clear stale content immediately; Nachtverlauf stays disabled until a safe read settles. Settled missing data can still open the honest empty night view.

Paper Phase3 HRV3I5L/4TO5 and RHR4TQK/4TSZ use canonical hero3I95, bars3I9I and leading row4U6E.43 exact JSX/PNG references were inspected, including all state variants and375px2x. Shared settings rows gained a36px leading tile and12px trailing gap without changing compact callers; shared InfoSheet gained grouped paragraphs and one pinned primary action. Scalar equality now says wie Basis, with no normal-range claim. Fourteen new golden PNGs inspected and committed; two superseded HRV goldens removed.

- Required Flutter3.41.6 OpenBand suite: **1891 passed**. Required analysis: **99 items, no issues**. Manifest: **127 blocks /690 screens**. Logs and inspected hashes: `build/night-scalar-acceptance/`. These final checks include the14px seven-night bars requested during review.
- Data45 focused tests; shared InfoSheet13 tests; final UI51 focused tests and5-file analysis passed. Fresh reviews0c8eda6a/38aa7b23 and6e2589de/b908e823 plus lead source/diff/log inspection closed version-anchor, source/timing, stale-source, footer, fixture-parity and in-flight navigation findings. Production metadata was not invented to fit synthetic designs.
- Dedicated Pro `build/ui-review/night-scalar-pro-20260921-v2`: **PASS123.03s**,393×852. Dedicated mini `build/ui-review/night-scalar-mini-20260921-v2`: **PASS114.27s**,375×812. **36 checkpoints each, all72 PNGs inspected**, run/frames/HTML reconciled. Covers light/dark,7/30/90, full/selected missing, partial, sparse, older, pending/failed/unproven/unreadable, baseline qualification, read failure/retry, Info, actual Nachtverlauf/back and correction destination,2x top/bottom.

Synthetic local/simulator proof only. No personal data, physical-device, Bluetooth or physiological validation. No schema, algorithm, pin or version change; nothing pushed. Shared overview/sleep/Health card receipt consistency and broader Health hub/method/quality flows remain open for the next unit.


## Shared night cards and history · 21 September 2026

Overview, Sleep and Health use the same selected-night HRV/resting-pulse policy as detail. SQL scalars, sleep/nap correction receipts and the correction banner are read in one transaction snapshot. Partial and older values remain labelled; pending, failed, unproven and outdated results are withheld. Comparisons require a complete current value and an explicitly trusted stored baseline. Health history uses the typed version-anchored reader and retains partial metadata. Its count caption labels partial input. A scalar baseline is one line, never an invented interval. The unused range component and its generic-detail caller are removed.

The shared card has one status line and adapts to large text. Paper includes24 inspected card/component/representative exports plus4 trend variants. Canonical card3ESH, adaptive pair3ESE, trend3ET0 and partial variants4XH1/4XHQ/4XKG are registered in blocks.json. All44 changed/new golden PNGs were inspected, including320px,375px2x and light/dark. Hashes: Mats task night-card-golden-review/accepted-44-hashes.json.

- Required Flutter3.41.6 suite: **1930 passed**; required analysis **100 items, no issues**. Logs: `build/night-card-acceptance/tests-v2.log` and `analyze-v2.log`. Final test-only navigation repair passed19 focused tests and scoped analysis including the integration target. No production changes followed the required suite.
- Fresh data reviewer0b1cda36/9341c226 inspected snapshot and receipt semantics. Synthetic date/override parity findings were corrected; lead additionally caught baseline-status normalization and historical partial metadata loss. Lead inspected integrated source, diffs and tests. Synthetic fixtures preserve48/54 values and unknown default baseline status.
- Dedicated iPhone15 Pro: `build/ui-review/night-cards-pro-20260921-v4`, **23 checkpoints, PASS83.18s**. Dedicated iPhone13 mini: `build/ui-review/night-cards-mini-20260921`, **15 selected checkpoints, PASS95.49s**. All38 PNGs inspected; run/frames/HTML/requested capture names reconciled. Covers all three roots, actual detail/back, light/dark, missing/partial/failed, generic strain after range removal, and2x cards. Filtering omits only screenshots; all interactions and assertions execute, and an unknown requested name fails.
...
### Native version metadata reconciliation — 2026-09-21

RunnerTests Debug/Release/Profile now match app0.9.31+67. Six changed setting lines; shipping widget/watch configs were already0.9.31/67. Runner retains FLUTTER_BUILD_NUMBER. Lead inspected exact diff and independently parsed all12 native target configurations with plutil JSON; no stale hardcoded65/0.9.29 remains. `plutil -lint` passes. Source verifies schema65,algorithm90,protocolfe1464db98b84ac4d3ce6175d54ada11356d6c62 andanalytics1fa8144a5e3b728ce91eeed6ecbc15d482933b44; full pins match pubspec.lock. App +BUILD preserved.

Required1944 OpenBand tests pass,101-item analysis clean,manifest127/691. Evidence build/version-metadata-acceptance/{tests.log,analyze.log,native-versions.json}. Author a13e645e/2a5bec05 implemented and verified in isolated version-metadata-clone; lead confirmed prior baseline byte-identical before integration. This is source/configuration proof, not a release archive, installation, watch execution or Bluetooth proof. No UI pixels changed; inspected Sleep native evidence remains applicable. No push.


### Respiration card and night detail — 2026-09-21

Health now opens the shared Atmung detail and actual respiration overnight curve. Published `scalars.resp_rate` stays authoritative; RSA brpm/peak_hz/power/source are provenance only. No uncertainty bounds exist in the writer and none are invented. Confidence has no default; input names stay typed strings. Same-result algorithm anchoring, receipt withholding, partial/missing/unreadable states and trusted-only baseline reuse the accepted night-scalar family. Decimal precision is shared by card, hero, baseline and comparison; HRV/RHR retain integer presentation.

Lead inspected production diff, critical parser/read paths and all14 changed goldens. DATA authorff054eb1/965b44e3, UI authorc3e786b2/d643adbb and shared decimal author2cc483bf/c50f1248+b5cc65d7. Fresh DATA reviewe1f8024d/1023eacd identified unsupported uncertainty and input coercion; correctionclosurebfa85735 verified both fixed. Fresh UI reviewfa8c875e/89e90178 found no blockers. Read-only reviewers did not execute tests. Lead reviewed the final RSA label delta.

Required suite **1971 passed**, **103-item analysis clean**, manifest **127 blocks /703 screens**. Logs `build/respiration-acceptance/`. All12 Paper references/exports in task `health-resp-paper`:4XTE/4XWH,4Y0E/4Y2S,4Y5G/4Y7W,4YAC/4YCV,4YGU/4YJE,4YNA/4YNL. All inspected; shared partial caption matches Flutter. No physiology, schema or dependency changes.

Native Pro `respiration-pro-20260921` passed72.71s, mini `respiration-mini-20260921` passed95.48s,21 checkpoints each. All42 PNGs inspected with run/frames/HTML reconciled. Main,7/30/90,missing/partial/error+retry, metadata, actual overnight navigation and2x top/bottom in both themes passed. Lead caught the standalone Health test host missing SafeArea; UI author83c43ea2 corrected only that fixture and card positioning. Replacement Pro `respiration-pro-health-20260921` passed62.4s and mini `respiration-mini-health-20260921` passed66.94s, twoPNG each, allfour inspected. Filtered captures retain every interaction/assertion. Original two Health-entry screenshots are superseded; unchanged detail evidence remains valid. Final fixture-only scoped analysis is logged separately. Native render matches the relevant Paper detail/card variants. Full Health hub remains open. No device/Bluetooth or physiological proof claimed.

## Exact-result source persistence, 2026-09-21

Schema65→66 adds nullable day_result.source with no default or backfill. Fresh creation, upgrade and same-version repair use the existing additive migration pattern. putDayResult stores source in the same transaction as its exact payload on every call; the date-only series stamp retains its previous rules. This prevents skin-temperature units from being inferred from another payload or algorithm version. No metric output, package pin, raw retention or BLE change.

Author6f6a25fd/65f3e30a and130c3231; independent read-only Sol reviewac307e90/f9fbfdab clean. Lead reviewed actual source/diff and realSQLite tests, including no-backfill migration/repair, partial/empty-series writes, null replacement and lower-version writes. Review notes no separate skipped-true or transaction-fault test; unconditional insert and existing transaction establish those paths without introducing branching. Existing additive helper error handling is unchanged.

Flutter3.41.6 required OpenBand suite1977 passed,103-item analysis clean;74 affected DB/migration/retention tests passed. Logs build/day-result-source-acceptance/{tests,analyze,db-tests}.log. Manifest127blocks/703screens and diff check passed. Nonvisual storage change, so no regenerated PNG or new simulator evidence. App0.9.31+67, algo90, schema66 verified from source. Full protocolfe1464db98b84ac4d3ce6175d54ada11356d6c62 and analytics1fa8144a5e3b728ce91eeed6ecbc15d482933b44 pins match pubspec.lock. Local acceptance only.


### Source-aware skin temperature · 2026-09-21

Health now opens the shared typed night detail for skin temperature. Exact day-result source distinguishes band SD from imported Celsius. Conflicting/unknown quantities remain unavailable in the headline and chart; Info retains raw values and both source channels. Same-algorithm, same-quantity history covers7/30/90 civil dates with real gaps. Correction receipts withhold stale values without deleting provenance. No ADC baseline, synthetic fallback, smoothing or overnight temperature route.

Lead inspected actual DATA/UI/integration diffs and tests. Fresh independent DATA d7ff2155/d3e1a279 and UI57a9cc67/da00622f reviews found no blocker. Production hashes match reviewed snapshots. Authors831e0808/c24ff4c4 and22a480ee/ccc0f2b1 plus integration repair3b3afc5a completed. Helper-only writer126f12ee/57ebaeaa extended exhaustive test switches without weakening assertions. Original DATA timeout2c066aed was reconciled with retained partial files and no live owned process; only successor work is accepted.

Required checks in build/temperature-acceptance:2011 tests pass,104-item analysis clean, integration analysis clean, manifest128blocks729screens, diff check clean. DATA focused evidence in private Mats task temperature-data-evidence includes42 pure tests,59 SQLite repository tests, focused receipt/source checks. No changes to production after required checks; only native test-helper repair followed.

All20 regenerated PNGs inspected:14 temperature and6 affected Health goldens, committed under test/openband_goldens. Final hashes equal temperature-golden-inspect/early-hashes.json. All42 actual native PNGs inspected with frames/run/index reconciliation: initial Pro13 captures stopped on a test-helper assertion requiring Quelle in a correct load-error state; repaired full interaction run temperature-pro-final-20260921 PASS141.04s adds8 remaining captures. temperature-mini-20260921 PASS106.82s has all21 states. Coverage includes SD/Celsius/unknown in both themes, source conflict/Info,7/90 periods, partial/missing/error/retry,2x top/bottom and Health entry/detail/back. Both runs use dedicated isolated synthetic gallery simulators. Initial Pro is partial image evidence only; final Pro supplies the successful interaction result.

Paper29 references exported and inspected in private Mats task temperature-paper/index.json: canonical OBNightLine4YO1/4YSR, cards4ZQX/4ZR8, SD/Celsius/unknown, missing/partial/error, source conflict, Info and375px2x variants. Partial heading refined to shared inline geometry and re-exported.26 screen refs plus canonical line block registered; existing Health root3FVE updated. Full Health hub remains partial in the checklist. Version0.9.31+67, schema66, algorithm90 and dependency pins unchanged. This is simulator/UI/storage proof, not physical Bluetooth or physiological validation.


### Dated journal weight · 2026-09-21

Health now opens the actual journal weight history with7/30/90 civil-day windows, the existing explicitly labelled EWMA, and field-level CAS entry/edit/removal. Undated profile weight is never a measurement fallback. Date, minute metadata and unrelated journal fields survive edits. Finite invalid trend inputs remain visible/editable but are excluded from the trend; malformed/nonfinite data is labelled, and valid future rows cannot taint a historical view. Missing dates stay gaps. Shared EWMA elapsed-day arithmetic now uses civil coordinates across DST; no analytics output/version change.

Saving and refresh are separate. A saved edit followed by read failure withholds stale values and offers a read-only retry. Conflict dismissal reloads the concurrent stored result. Returning to the retained Health tab refreshes through the existing controller request counter. Shared value rows, editor, calendar and line chart were reused/extended; OBNightLine callers migrated to OBCalendarLine and the old file removed. Calendar preserves actual today while bounding dates by min(selected day,today), with year1 reachable.

Lead inspected actual source/diff, meaningful test failures and final evidence. DATA reviewd420977b/22d68125 clean after premature date filtering correction; UI reviewda0883ab/fb86682d closes stale-conflict and refresh paths; calendar review97e91750 clean. Main UI author011f1049 finished the error-observation/spacing corrections and final ink retry actionf74fc488. Calendar authorb86764ff timed out with preserved partial code; host tests and independent review accepted the frozen diff. Redundant successor6a2607f9 was stopped and reconciled with no additional delta and no surviving process. No completion was inferred from timed-out workers.

Flutter3.41.6: **2092 required OpenBand tests passed**, **110-item analysis clean** including the integration file, manifest **129 blocks/755 screens**, diff check clean. Logs build/weight-acceptance/{tests-accepted,analyze-accepted}.log. Focused DATA39 passed, DST31 passed with TZ=Europe/Berlin, calendar/UI39 passed. All6 regenerated weightgoldens inspected and committed; existing temperature goldens passed unchanged.

Native Pro weight-pro-final-20260921 PASS79.04s/28 checkpoints; final ink retry correction weight-pro-retry-20260921 PASS72.76s/3 replacement captures. Mini weight-mini-20260921 PASS107.53s/28 checkpoints. All59 PNGs inspected and run/frames/HTML reconciled. Light/dark, actual Health navigation/return, missing/partial/unreadable,7-day trend, CRUD, conflict, read/save/saved-refresh failures, calendar bounds and375px2x editor/entries are covered. The initial weight-pro-20260921 failed on an obsolete harness401 write;14 initial images were inspected but that run is not accepted interaction evidence. Final fixture uses explicit synthetic seeding without weakening production validation.

Paper30 references exported/inspected in private task weight-paper/index.json: canonical line51YA/51YU; history50BT/50FU, edit50IN/50M2, empty50PG/50S9, loaderror50V2/50XV, saveerror5110/514E, Info517W/51AP,375px2x51E8/51H1, new51KK/51NF, conflict51QA/51TP, unreadable51ZO/522J, saved-refresh52DC/52G7, partial52JA/52M3, calendar52X0/5305; Health row3G62 reused. Spacing and retry control matched to native. Version0.9.31+67, schema66, algorithm90 and full dependency pins unchanged. Synthetic simulator/storage proof only.

This is the completed journal subset of B152. Source-preserving Health weight observations, multiple-observation history, date-effective explicit profile adoption and affected recalculation remain open; the checklist keeps those states partial. No push.


### Manual VO2max and import receipts · 2026-09-21

Health opens dated manual VO₂max values with separate measurement day, entry/revision times and declared method. Schema67 adds immutable revision chains, stable creation IDs, CAS edit, removal/restoration and idempotent retry. Corrupt heads never fall back to older values. Saved edits survive refresh failures; reload never repeats a committed write. No physiological estimator, invented trend or reference classification. B150 classification remains partial because a versioned licensed reference catalog is absent.

Backup import appends compatible revision chains and withholds conflicting/corrupt IDs independently. Typed interrupted-import receipts retain only committed counts, including earlier files/tables and the known presence of the VO₂max table. Failed pages/IDs roll back. Empty, unchanged, partially imported and interrupted are distinct. CSV/range backup preserves whole revision chains; ordinary day deletion retains these manual records, explicit wipe clears them. Existing source retention, analytics and commit-before-ACK are unchanged.

Lead inspected the integrated source/diffs and accepted corrections from storage453b5569/35948acf, adaptersa9339334/14c35c76, UI fdec77fd/a4ef529f, receipt6ed86d1b/4bc940c1 and native75b60abf/42f80406. Independent reviews3111f178/52316d54,1a81ce83/3608ca1c,59c13272/d3390704 and9ea9f761/3187216d found issues that were returned to their owners and corrected. Host review additionally fixed the Health return callback returning a Future, migrated two interrupted-import expectations while retaining their storage assertions, and corrected native selectors for nested field scrolling and large-text value wrappers.

Flutter3.41.6: **2188 required OpenBand tests passed**, **114-item analysis clean**, native-test analysis clean. Affected storage/import/migration/export integration113 passed; final receipt/mixed-import40 passed; final VO₂max UI23 passed; cycle/weight integration repairs61 passed. Logs: `build/vo2-acceptance/`. Manifest **132 blocks/795 screens**, diff check clean. All nine new goldens inspected and committed under `test/openband_goldens`; hashes recorded in `inspected-golden-hashes.json`. No `test/goldens` directory was created.

Paper references are registered in blocks.json;40 current-unit screen references reuse the canonical VO₂max detail/editor, settings rows, calendar and import receipt. Exports and inspected JSX/PNGs are in the Mats task `vo2-paper/index.json` and `vo2-receipt-paper/index.json`. Source, missing/partial/error, edit/history, conflict and light/dark variants are covered. Native receipt review exercises the production ImportReport component in a synthetic host; it does not claim the entire A08 import route has been migrated. Full Health hub parity remains open.

Native acceptance: Pro `vo2-pro-pass-20260921` passed77.93s with every VO₂max/import interaction and assertion; capture filtering only skips screenshots. The44 intended Pro state images are in `vo2-pro-complete-20260921` (27) and `vo2-pro-verified-20260921` (17), plus one final editor replacement in the passing run. All45 inspected. Earlier runs stopped on test-selector defects; the last image run completed interactions but rejected two requested optional bottom captures because the content already fit. These failed run records are preserved, and the separate final passing run establishes interaction acceptance.

Mini `vo2-mini-accepted-20260921` passed118.35s with all interactions and19 selected captures. All19 PNGs inspected:375×812, light/dark, missing/partial/unreadable, draft/error/conflict, calendar and2x value/editor/receipt layouts. All64 final Pro/mini PNGs have inspected hashes; run/frames/HTML agree. No physical-device, Bluetooth or physiological validity proof is claimed. App0.9.31+67, algorithm90 and full dependency pins are unchanged; protocolfe1464db98b84ac4d3ce6175d54ada11356d6c62 and analytics1fa8144a5e3b728ce91eeed6ecbc15d482933b44 match the lockfile. Widget/watch/RunnerTests metadata remains0.9.31/67. This unit is the user-requested stopping point before a read-only scope consultation, not completion of the entire checklist.

## 21 September 2026 · Existing device export and reduced release

Mats supplied the full database in Downloads. The newest export is schema54, 126,320,640 bytes. Read-only SQLite inspection passed; an isolated copy migrated to schema67 and a complete `VACUUM` export retained all table counts. The original file hash remained unchanged. Private inventory, hashes and test databases are under `~/Library/Application Support/OpenBand5Lab/release-review-20260921`; no private records are committed. The harness and log are in `/private/tmp/openband-release-reduction-20260921/device_copy_test.dart` and `device-copy-test.log`.

The snapshot contains 505,413 decoded Gen5 seconds, 253,993 beat rows, 16,315 retained raw archive rows and 10,428 acknowledged sync-ledger rows. Fifteen stored day-result rows span algorithm versions87–90. One acknowledged ledger row retains a prior validation-failure/retry trace. This is evidence of actual capture, retained data and a recorded retry; it does not establish the chronology of an intentional phone-kill/disconnect trial, zero data loss, or physiological validity. Older notes saying no physical evidence was available predate this supplied export. No new phone or Bluetooth session was opened.

The initial restore test retained decoded seconds/beats/raw archive and the legacy sleep override, but omitted the saved correction/job and alarm schedules. Its passing inventory harness is not restore acceptance. A dedicated restore repair and second private-copy roundtrip remain required.


## 22 September 2026 · Reduced release navigation

Production opens one Übersicht domain without a bottom bar. Saved tabs, parked notification routes/schedules/buzzers and gestures cannot reopen Training, Journal or intake; saved preferences and data remain intact for explicit development builds. Retained alarms, stale-sync and health alerts still emit, and an existing active session remains finishable from Home. Overview, four stored band metrics and the compact Steps card use the Release Paper compositions. No metric algorithms, protocol pins or schema changed.

Independent SWE-2 review findings were corrected by the implementation owner, then the lead inspected the actual diff. Flutter3.41.6:2206 required OpenBand tests and131 affected notification/AppState tests passed; required analysis clean. Logs are `/tmp/openband-release-checks/final-{openband,affected,analyze}.txt`. Manifest passes58 blocks/26 screens. No golden PNG changed.

Native runs `release-nav-pro-20260922` and `release-nav-mini-20260922` passed with20 inspected images across light/dark, missing/error and2x. `release-large-bottom-20260922` passed and its mini2x lower-content PNG was inspected. `release-missing-verified-20260922` passed and both Pro missing/error PNGs were inspected. Native-test analysis passed. All paths are under `build/ui-review/`; run.json/frames.json record synthetic evidence. The lead initially misread repeated image previews as missing native status bars. Direct bitmap inspection confirms the native bar in every flagged original PNG. There was no demonstrated app inset bug; speculative capture retries and extra SafeArea wrappers were rejected. One intermediate diagnostic run failed on object-identity comparison in a temporary assertion; it is not acceptance evidence. The retained harness change preserves the original failure instead of masking it with a missing-capture error.

Paper Release/shared pages are first; archives remain intact. The release page contains24 representative boards and32 shared block boards. Fixture-specific rings and charts preserve real stored samples and omit unsupported baselines. Backup/restore, compact Profile/Data and Band setup repairs remain open; navigation completion is not overall release completion. Nothing was pushed.


## 22 September 2026 · Durable database restore

Database restore and rebuild salvage now retain saved sleep corrections/jobs/drafts, templates/pins, meal drafts, alarms, sessions/laps/details, dismissed suggestions, live coverage and the raw replay ledger. A differing destination edit remains local; revision numbers from separate databases do not establish ancestry. Sleep families commit atomically, newly restored corrections queue calculation, and an identical completed job stays completed. Child records require the matching source parent. Newer source columns are filtered to the destination schema. Real read/write interruption receipts report only committed records. Existing decoder and algorithm behavior is unchanged.

Fresh SWE-2 review identified alternate-key collisions, incomplete interruption receipts, forward-column compatibility, salvage omissions and session-parent consistency. The responsible implementation owner corrected them; the lead inspected the actual source and integrated six matching files. Final evidence:2214 required OpenBand tests,63 affected backup/recovery/database tests,115-item analysis, manifest59 blocks/28 screens and diff check passed. Logs `/tmp/restore-swe2-{required-tests-final,required-analyze-final,postcallback}.log`. No golden PNG changed. Earlier whole-suite diagnostics also found unchanged parked nutrition accessibility overflows and a caffeine admission-source guard; that broad suite is not claimed green.

The final private-copy test passed in9s after the separately verified schema54→67 migration. Full snapshot counts match; fresh restore retains all checked durable tables, including sessions, correction1/job1/alarm2/live coverage107/suggestion1/decoded505413/beats253993/archive16315. Restored jobs remain pending; SQLite quick/FK checks pass. The original Downloads export hash is unchanged. Private artifacts remain in OpenBand5Lab; the final harness/log and integrated hashes are in `/private/tmp/openband-release-reduction-20260921`. This does not claim successful recalculation or new Bluetooth operation.

Paper receipt light/dark569H/56A9 reuses the canonical import card. Native `restore-receipt-pro-20260922` passed with3 inspected light/dark/2x PNGs; `restore-receipt-mini-20260922` passed with1 inspected2x PNG. Run/frame inventories agree. The final receipt source hash equals the captured version, so no duplicate capture was needed. Saving, conflicts, unreadable input and pending calculation have separate concise lines. Compact Profile/Data and Band recovery remain next. Nothing was pushed.


## 22 September 2026 · Compact Profile and data management

Production Profile and Data now use the compact Release Paper composition. Identity, Band, settings and language retain their real destinations. Database/encrypted/CSV exports, import, backup cadence, immediate backup and recalculation remain connected to production actions. Export creation is reported separately from sharing; a saved schedule remains saved if its first backup fails. Failed preference persistence restores the prior schedule. Phone permissions can still be revoked; parked glucose remains available only in the explicit development surface. No source or algorithm behavior changed.

The synthetic gallery mounts those production views with isolated callbacks, including export failure/retry and cadence changes. It opens no private database or Bluetooth session. Two gallery tests previously stalled together because asset I/O ran inside the fake clock; loading through tester.runAsync fixed the cause without removing assertions. The existing glucose test now verifies the reduced gate and retains its full development-mode assertions.

Lead inspected the integrated ten source/test files and fresh read-only Devin SWE-2 review. The reviewer found no blockers; native inspection caught broken large-text Band facts, which the implementation owner corrected to the canonical stacked rows. Final matching source hashes, process drainage and actual check outputs are recorded in `/private/tmp/openband-release-reduction-20260921/profile-finish-evidence.json`. The worker timed out before its final narrative, after both required commands completed successfully; acceptance uses the inspected command results and artifacts.

Flutter3.41.6: **2225 required OpenBand tests passed**, **115-target analysis clean**, and **28 combined release-scope tests passed**. Logs `/tmp/profile_data_openband_tests_required_final.log`, `/tmp/profile_data_analyze_required_final.log`, `/tmp/profile_data_release_scope_final.log`. Manifest59 blocks/28 screens and diff check pass. No golden PNG was regenerated.

Native Pro `profile-data-pro-20260922` passed64.33s with9 inspected PNGs. Its initial large Profile image was rejected and replaced by the inspected `profile-large-pro-final-20260922` image, passed63.38s. Mini `profile-data-mini-20260922` passed75.57s with7 inspected PNGs. All17 generated images were inspected; the accepted16 cover light/dark,375px,2x scrolling, export failure/retry and cadence interaction. Run/frames inventories agree. Final archive-mode propagation changes do not alter the captured reduced views. This is synthetic simulator proof, not new physical/Bluetooth or physiological proof.

Paper Profile3GQP/5619 and Data55UX/55Y8 have canonical64-point headers and44-point controls. Shared board3GQQ includes52-point rows,60-point subtitle rows and the inspected large-facts light/dark variants56DX/56EA; no extra screen matrix. Native unknown stored time remains a dash. App/widget/watch0.9.31+67, schema67, algorithm90 and full protocol/analytics pins match source/lock. Band setup/recovery is the remaining implementation unit. Nothing pushed.


## 22 September 2026 · Band setup and recovery

Release pairing and re-pair now enter the canonical WHOOP view directly. Root exits update the app gate; pushed re-pair closes exactly once. Accessory authorization without a persisted paired device is cancellation, not success. The secondary-sensor picker remains available and distinguishes a failed scan from a successful empty scan. Permission cards show the cause and required action; the longer reason is behind Info. Generic privacy filler was removed.

First transfer retains the actual stored-through time during interruption. Resume re-reads the real session state because the orchestration method can return after handling a failure internally. Disconnected or interrupted remains retryable; connecting stays pending; a later actual connection clears stale failure. A missing evaluation stays a dash. Root first-transfer has no inert Back control. Injected synthetic reads cannot implicitly open a production Bluetooth session. Shared interactive sync rows have44-point targets and stack at large text.

The lead integrated and inspected the bounded Sol implementation after both routine Grok routes stalled and were drained. Fresh read-only Devin SWE-2 source review found no blockers. Lead native inspection rejected verbose permission copy and a mid-word large-text label; the responsible worker corrected both. Final integrated source and check artifacts are under `/private/tmp/openband-release-reduction-20260921`. No protocol, analytics, database schema or dependency behavior changed.

Flutter3.41.6: **2226 required OpenBand tests passed**; **115-target analysis clean**. Required logs: `band-required-final.log`, `band-analyze-final.log`. Focused owner checks cover scan errors, direct onboarding/back, state-based resume, permission disclosure and large-text action placement. All12 changed/new golden PNGs were inspected, including the two interrupted development-shell overview fixtures; no `test/goldens` directory was created. Manifest **59 blocks/28 screens** and diff check pass.

Paper Release has28 representative screens; shared has32 boards. Pairing light/dark, root header, concise permission variants and the large-text sync row reuse existing canonical boards. The stale decorative transfer-progress track was removed because no real progress value supports it. Two nonexistent registry aliases were removed; their actual destinations remain. Historical pages and Designphase2 remain intact.

Native Pro `band-pro-20260922` passed66.63s with16 inspected PNGs. Six were rejected for copy/large-text issues and replaced by the six inspected images in `band-pro-final-20260922`, passed65.33s. Mini `band-mini-final-20260922` passed76.09s with10 inspected PNGs. All32 generated images were inspected;26 are accepted evidence. Run/frames/HTML inventories agree. Coverage includes light/dark,375px,2x scrolling, permission failure, interrupted transfer, a returned-but-failed resume and successful retry without fabricated evaluation. The owner-corrected Paper dimensions, card/header structure and spacing were compared with actual native renders. These are isolated synthetic simulator runs, not a new physical session.

The seven reduced-release checklist outcomes are now evidence-backed. This completes the approved local review endpoint; parked full-product areas remain preserved and are not claimed implemented by this release. App/widget/watch0.9.31+67, schema67, algorithm90 and full protocol/analytics pins match source and lock. The supplied private database establishes capture/retention and recorded retry, not controlled phone-kill chronology, zero loss or physiological validity. The earlier broad diagnostic still has three unchanged parked nutrition failures; only the required and affected suites above are claimed passing. No push, merge, deployment or physical Bluetooth operation occurred.


## 22 September 2026 · Physical iPhone release installation

Mats explicitly authorized building and installing on the connected iPhone. Built commit256982f4 with Flutter3.41.6 using `build ios --release --no-pub --target lib/main.dart --dart-define-from-file=.env`. The signed production build completed (76-second Xcode build,53.9MB); strict deep codesign verification passed. Artifact metadata is `dev.matsvarn.openband5`,0.9.31+67. The generated target is main.dart, with no synthetic review or development-scope defines. Effective protocol/analytics package roots resolve to the pinned Git commits. Nothing was pushed or published.

Before installation, stopped only the identified OpenBand process and copied its Documents database/WAL plus preferences into private OpenBand5Lab storage. A standalone SQLite backup passed quick/FK checks. Despite matching installed version metadata, that live database was schema54. The current LocalDb migration/export/restore harness passed on a fresh isolated copy in12s; existing retained-source counts were unchanged. The first count-comparison probe expected raw_records to pre-exist, then was corrected to recognize this newly created empty table. No original database was edited.

Installed in place through devicectl without uninstalling, then launched the app. iOS assigned a new data-container path; path identity was therefore not used as retention proof. The post-launch copy confirms actual on-device schema67, passing quick/FK checks, retained source/session/override counts and six unchanged pairing/profile/onboarding preferences. The original raw archive count is retained. The physical overview renders stored data and reports band reading. A user picture-in-picture overlay obscures part of the header, so this screenshot is not full-screen visual acceptance.

Private backups, bundle hashes, install/launch receipts, database inventories and actual screenshots are under `~/Library/Application Support/OpenBand5Lab/device-install-20260922-256982f4`. A bounded read-only Devin SWE-2 review found no source blocker; the lead rejected and corrected its inference that equal app versions imply equal installed schema. Actual-device evidence above is distinct from the isolated-copy test. This installation is not a zero-loss interruption trial, overnight reliability study or physiological validation.

A normal terminate-existing/relaunch succeeded with a new running process. A later device copy still passes schema67/integrity checks and contains107 additional decoded seconds and41 additional beat rows compared with the first post-install copy; archive/session/override counts remain unchanged. This supports successful relaunch and continued storage during this short observation. The later screenshot showed another foreground app and was deleted rather than retained as OpenBand evidence. No navigation or permission settings in other apps were changed. The app remains installed and running; a controlled mid-transfer disconnect/zero-loss trial remains separate.

## 22 September 2026 · Algo-92 capture build on the physical iPhone

Built commit `a7d9b9ca` (algo92) with Flutter3.41.6, `build ios --release --target lib/main.dart --dart-define-from-file=.env` (36.9 s Xcode build, 53.9 MB), installed in place through devicectl and launched from the new bundle. Post-launch full-Documents copy (db+wal+shm) verifies: schema67 unchanged; `band_backlog` cursor columns present and populated on the live connect — `read_page`101536, `raw_old_page`101568, `current_read_ts`1790072293 (≈sync instant), `trim_ts`1790072248, `wrap_count`13 stable. The read cursor sitting AT the live edge is the stored fact that answers the backlog question: the Apr–Sep strap window is behind the cursor and unreachable by normal `requestHistorySync()`; the gap is accepted per the agreed decision, and no `SET_READ_POINTER` was sent or is planned.

Zero-loss state on this copy: `decoded_onehz`553,697 (+9,448 since the previous pull), `raw_blob`188 batches/9,448 records with every blobbed timestamp present decoded and every declared `n` matching its inflated frame count; `tool/replay_check.dart` re-decoded all 9,448 blobbed frames through `decodeSubstrate` — 0 hr/temp/step mismatches, 0 seconds missing from the store. `sync_ledger` shows 0 `acked_at < created_at` violations since the fix boundary (5,546 historical, all predating install). `tool/verify_capture.py` verdict: CLEAN. Eight day_results still carry algo91 — v92 re-derivation had not yet run at pull time. Evidence under `~/Library/Application Support/OpenBand5Lab/device-install-20260922-a7d9b9ca`. This remains install + post-install verification, not the scripted interruption trial and not physiological validation.

## 22 September 2026 · Post-audit fix release on the physical iPhone

Built commit9e58f5d8 (algo91) with Flutter3.41.6 using `build ios --release --no-pub --target lib/main.dart --dart-define-from-file=.env`; signed build succeeded (43-second Xcode build,53.9MB), strict deep codesign verification passed, artifact `dev.matsvarn.openband5`0.9.31+67. The only running OpenBand process was terminated and the app-data container (Documents DB/WAL/SHM) copied to private OpenBand5Lab storage first; that backup passed integrity at schema67 with544,249 decoded seconds. Installed in place through devicectl; launch initially refused on a locked device and succeeded after unlock.

Post-launch copy: schema67 unchanged (additive self-heal path, no bump), integrity ok, decoded_onehz grew to544,358 (+109 seconds committed post-install) and raw_archive unchanged at17,582. The new `raw_blob` table exists and already holds3 batches totaling the same109 records; each payload inflates to contiguous0x2F v18 frames with counters23294861–23294969, byte-verified against the committed range. All eight days re-derived at algo91 with the stamped build block `{algo_version:91, analytics_pin:1fa8144…, protocol_pin:fe1464d…, schema_version:67}`. Steps now publish the on-chip counter: Sep17 16,348 (was 4,621), Sep18 6,187 (was87), Sep19 9,468 (was190), Sep20 8,582 (was685), Sep21 10,335 (was2,947) — `source:strap_counter` with the suppressed windowed totals retained under `real_measured`. Evidence under `~/Library/Application Support/OpenBand5Lab/device-install-20260922-9e58f5d8`. This is not a disconnect/zero-loss trial or physiological validation; a controlled mid-transfer test remains separate.
